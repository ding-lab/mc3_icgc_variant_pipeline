...
variant_pth <- 'full6.tsv'
df_variant <- fread(variant_pth)
gene_model_pth <- 'general_gene_model_annotation.6.tsv'
...
uu_p <- fread("/diskmnt/Projects/ICGC_MC3/Gen_Figures/InverseControlledMAF_Figure/unique_icgc.notin.mc3_controlled.txt")
...
pdf("Proportional_genemodel.pdf",height=4,width=9,useDingbats=F)
print(gmp)
dev.off()


pdf("Count_genemodel.pdf",height=4,width=9,useDingbats=F)
print(gmn)
dev.off()
