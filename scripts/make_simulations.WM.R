...
#args <- c("output/full_cleaned.tsv","processed_data/purity_ploidy.txt","processed_data/tcgaPurity.csv","processed_data/CADD.annotations.fullvars.txt")
###@@@########################@@@###
...
maf = fread(input=args[1], sep="\t", header=TRUE, na.strings="NA",colClasses=list(character=c(128,135,139)))
#maf is  202459 rows 

#maf = fread(MAF , sep="\t", header=TRUE, na.strings="NA",colClasses=list(character=c(128,135,139)))
purity = read.csv(args[2], sep="\t")[,1:2]
tcgaPurity = read.csv(args[3], skip=3)
cadd = as.data.frame(fread(args[4],colClasses=list(character=c(9))))
...
#maf is 124604 lines now


# Designate variant capture status
maf$both = ifelse(!is.na(maf$Chromosome) & !is.na(maf$"Chromosome:1"),1,0)
maf$p_only = ifelse(is.na(maf$Chromosome) & !is.na(maf$"Chromosome:1"),1,0)
maf$t_only = ifelse(is.na(maf$"Chromosome:1") & !is.na(maf$Chromosome),1,0)
...
for(i in 1:length(upt)) {
  myPt = upt[i] #get samples id
  ix = which(maf$mc3_exome_barcode == myPt) #get locations of maf where 
  pot_reads = maf$p_tot[ix] # create a vector of PCAWG read depths 
  clear_reads = pot_reads[!is.na(pot_reads)] #remove missing
  if(length(clear_reads)==0) {
    clear_reads = 0
  }
  maf$p_simReads[ix] = sample(clear_reads, length(ix), replace=T) #Randomly put a read depth per variant by sample
}
maf$p_simReads[!is.na(maf$p_tot)] = maf$p_tot[!is.na(maf$p_tot)] #Put the missing values back
...
cadd$uvar = paste(cadd$CHR, cadd$POS, cadd$ALT, sep="_")
